---
title: "R Notebook - Analysis Immune cell types"
output:
  html_document:
    df_print: paged
---
...
immune.sce <- readRDS(file=file.path(data_folder, "IMMUNE_CLINICAL-DATA_FILTERED.rds"))
...
tbl[tbl$`Immune cell number overall` <=100,] #97 patients have less than 100 Immune cells=10%.

all_immune_pat <- tbl[tbl$`Immune cell number overall` <=100,]$`Patient ID`
...
tbl[tbl$`Immune cell number overall` <=50,] #214 images have less than 50 Immune cells (10% of median)
#Lowest 10% of all patients' Immune cell numbers
tbl[order(tbl$`Immune cell number overall`),]$`Immune cell number overall`[1:200] #1-47
...
saveRDS(immune.sce_pat.roi,file=file.path(data_folder, paste("Immune_all_sce_pat_roi_rem.rds",sep="")))
saveRDS(immune.sce_roi,file=file.path(data_folder, paste("Immune_all_sce_roi_rem.rds",sep="")))
saveRDS(immune.sce_pat,file=file.path(data_folder, paste("Immune_all_sce_pat_rem.rds",sep="")))
...
tdat <-as.data.frame(table( immune.sce_pat[, immune.sce_pat$Patient_ID!="Control"&
                            ...
                            immune.sce_pat[, immune.sce_pat$Patient_ID!="Control"&
                                           immune.sce_pat$DX.name=="Adenocarcinoma"|
                                           immune.sce_pat$DX.name=="Squamous cell carcinoma"]$immune_type))
...
plot(plt)
#ggsave(plot=plt, file=file.path(plot_folder, paste("Barplot_n_hc_immune_type_proportions.pdf")), width=6, height=6)
```
...
surv_dat$Tcell_G <- ifelse(surv_dat$Tcell >summary(surv_dat$Tcell)[3],"Tcell high","Tcell low")

surv <-c("Bcell_G","Neutrophil_G","Myeloid_G","Tcell_G", "metacluster")
```
...
res.cox <- coxph(Surv(time, status) ~   Neutrophil+Bcell+Tcell + Myeloid + Stage+Grade+M.new, data =  surv_dat)
...
#OS survival
surv <-c("Bcell_G","Neutrophil_G","Tcell_G","Myeloid_G", "metacluster")
...
surv_dat$Tcell_G[surv_dat$Tcell >summary(surv_dat$Tcell)[5]] <-"Tcell high"
surv_dat$Tcell_G[surv_dat$Tcell <=summary(surv_dat$Tcell)[2]] <-"Tcell low"
surv_dat$Tcell_G[surv_dat$Tcell >summary(surv_dat$Tcell)[2]& surv_dat$Tcell<=summary(surv_dat$Tcell)[5]] <-"Tcell medium"

surv <-c("Myeloid_G","Neutrophil_G","Bcell_G" ,"Tcell_G","metacluster")
```
...
for (i in (categories)) {
    ...
    t <- table(df$immune_type, df$Patient_ID)
...
tdat <-as.data.frame(table(immune.sce_pat.roi[,immune.sce_pat.roi$Patient_ID!="Control"&
                           ...
                           immune.sce_pat.roi[,immune.sce_pat.roi$Patient_ID!="Control"&
                                          immune.sce_pat.roi$DX.name=="Adenocarcinoma"|
                                          immune.sce_pat.roi$DX.name=="Squamous cell carcinoma"]$immune_type))
...
surv_dat$total_density <- rowSums(surv_dat[,2:5])
...
res.cox <- coxph(Surv(time, status) ~   Myeloid+ Bcell+ Neutrophil +Tcell+ Stage+Grade+M.new, data =  surv_dat)
...
#Not split
#surv <-c("metacluster")
surv <-c("Neutrophil_G","Bcell_G","Myeloid_G","metacluster", "Tcell_G","Density_G")
library(survminer)
...
#pdf(file=file.path(plot_folder, paste0("Survival_Density-immune_type_hi-low_OS over",i,"_per_",k,".pdf")))
...
#pdf(file=file.path(plot_folder, paste0("Survival_Density-immune_type_hi-low_DFS over",i,"_per_",k,".pdf")))
#print(p,newpage=FALSE)
#dev.off()
...
surv <-c("Bcell_G","Tcell_G","Myeloid_G","Neutrophil_G","Density_G","metacluster")
```
...
#Not split
surv <-c("Bcell_G","Myeloid_G","Neutrophil_G","Tcell_G","Density_G","metacluster")
library(survminer)
...
p <-ggsurvplot(km_trt_fit,
 ...
 # facet_grid(.~surv_dat[[i]])
print(p)
#pdf(file=file.path(plot_folder, paste0("Survival_Density-immune_type_hi-med-low_OS over",i,"_per_",k,".pdf")))
#print(p,newpage=FALSE)
#dev.off()
...
#pdf(file=file.path(plot_folder, paste0("Survival_Density-immune_type_hi-med-low_DFS over",i,"_per_",k,".pdf")))
#print(p,newpage=FALSE)
#dev.off()
...
surv <-c("Bcell_G","Neutrophil_G","Myeloid_G","Tcell_G","Density_G","metacluster")
...
dat$Tcell_G[dat$Tcell >summary(dat$Tcell)[5]] <-"Tcell high"
dat$Tcell_G[dat$Tcell <=summary(dat$Tcell)[2]] <-"Tcell low"
dat$Tcell_G[dat$Tcell >summary(dat$Tcell)[2]& dat$Tcell<summary(dat$Tcell)[5]] <-"Tcell medium"
...
```{r, KW W Immune cell category density WO neo,warning=FALSE, message=FALSE, echo=FALSE, fig.width=12, fig.height=6}
...
tdat <-tdat_wide %>% pivot_longer(cols=c(colnames(tdat_wide[,2:6])), names_to = "Phenotype",values_to = "density")
...
```{r, KW Wilcox Immune cell Category density W neo,warning=FALSE, message=FALSE, echo=FALSE, fig.width=12, fig.height=6}
#define variables to be tested here
categories <- c( "Dist.Met", "T.new","M.new","N","Relapse","DX.name","Stage","Grade","Gender")
